...
//срок хранения, а равно, и время жизни - все время выполнения программы, т.е. статическое
...
int main(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 5; j++) {
            ...
            //время жизни автоматичское => пока выполняется блок, в данном случае - итерация

...
<span style="color:violet">__stddef.h__</span> - файл, который определяет макросы и типы, используемые для работы с размерами, указателями и смещением элементо<br>
<span style="color:violet">__stdalign.h__</span> - файл, который был добавлен в стандарт C 11 (ISO/IEC 9899:2011) и предоставляет макросы для работы с выравниванием данных<br>
...
<span style="color:violet">__fmod(double __x, double __y)__</span> - метод из библиотеки `<math.h>`, для взятия остатка с плавающей запятой
    
<span style="color:violet">__pow(double base, double exponent)__</span> - метод из библиотеки `<math.h>`, для возведения в степень <span style="color:orange">_exponent_</span> числа <span style="color:orange">_base_</span>. Работает в т.ч. с числами с плавающей запятой

<span style="color:violet">__abort()__</span> - метод из библиотеки `<stdlib.h>` для преждевременного завершения программы
...
<span style="color:violet">__CHAR_BIT__</span>
1. макрос (константа), объявленный в заголовочном файле <span style="color:orange">_limits.h_</span>
2. Обозначает количество битов в байте
...
<span style="background-color:red; color: black">__АЛАРМ!__</span> В обратном коде биты значения <span style="color:orange">_инвертируются_</span>, т.е. если -1 в прямом коде это 10000001, то в обратном коде это значение будет иметь вид 11111110. А в <span style="color:orange">_дополнительном коде_</span> необходимо сложить значения <span style="color:orange">_обратного кода_</span> и <span style="color:orange">_1_</span>, и, таким образом, для -1 значение в дополнительном коде будет равно 11111111.
...
<span style="color:violet">__Порядок__</span> - степень порядкового числа в экспоненциальной записи (например, для -6,022 x 10^23, 23 - это порядок). Чтобы не хранить отдельно знак порядка, применяют <span style="color:violet">__смещение__</span> (<span style="color:violet">__bias__</span>). К реальному порядку прибавляют константу, и в память записывают эту сумму. Для float смещение равно 127 (следовательно, чтобы представить порядок 0, в поле экспоненты сохраняется 127; порядок 73 сохранится как 73+127 = 200). Для double смещение равно 1023. Крайние значения экспоненты (точнее ее записи, все нули и все единицы) зарезервированы для специальных случаев (денормализованные числа, бесконечность, NaN).

<span style="color:violet">__Мантисса__</span> - значащие цифры числа.

> Важная особенность стандарта IEEE 754: для нормализованных чисел целая часть мантиссы всегда равна 1 (поскольку число приводится к виду 1.xxxx…). Эта единица не хранится в памяти явно, а подразумевается – это так называемый <span style="color:orange">_неявный бит_</span>. Благодаря ему фактическая точность мантиссы на один бит больше, чем количество выделенных бит в поле мантиссы.
...
- S - знак (для всех типов - 1 бит) 
- M - мантисса (для float - 23 бита, для double - 52 бита)
- E - порядок (для float - 8 бит, для double - 11 бит)
...
1. Знак 0 – число положительное.
2. Порядок 10000001₂ = 129₁₀, реальный порядок: 129 – 127 = 2.
3. Мантисса 010…₂ – это дробная часть. Полное значение с неявной единицей: 1.01₂ (в двоичной системе).
4. Таким образом, число равно 1.01₂ * 2² = 101₂ = 5₁₀.

Ключевые особенности стандарта IEEE 754:

1. <span style="color:orange">_Денормализованные числа_</span> — когда порядок равен нулю, неявный бит становится равным 0. Позволяют представлять значения, близкие к нулю, ценой снижения точности
2. <span style="color:orange">_Специальные значения_</span> — включают представления для ±∞ и NaN (Not a Number). Кодируются с помощью зарезервированных значений порядка (все единицы) и особых комбинаций мантиссы.
...
Стандарт IEEE 754 определяет несколько форматов представления чисел с плавающей точкой, которые различаются по размеру и, соответственно, диапазону и точности. (диапазон указан с учетом денормализованных чисел)

| Формат                   | Размер (биты) | Диапазон (приблизительно)     | Точность | Мантисса | Экспонента(порядок) | Типичное применение                 |
| ------------------------ | ------------- | ----------------------------- | -------- | -------- | ------------------- | ----------------------------------- |
| binary16 (half)          | 16            | ±6.55×10^-8 до ±65504         | ~3.3     | 10       | 5                   | Машинное обучение, графика          |
| binary32 (single, float) | 32            | ±1.4×10^-45 до ±3.4×10^38     | ~7       | 23       | 8                   | Общее программирование              |
| binary64 (double)        | 64            | ±4.9×10^-324 до ±1.8×10^308   | ~15-16   | 52       | 11                  | Научные расчёты                     |
| binary128 (quad)         | 128           | ±6.5×10^-4966 до ±1.2×10^4932 | ~34      | 112      | 15                  | Финансовые вычисления, криптография |
...
Точность (количество значащих бит мантиссы, с учетом <span style="color:orange">_неявного бита_</span>):
1. float — 24 бита
2. double — 53 бита
...
errno_t do_stuff(signed long value) { //для переносимого кода лучше тип int
...
# <span style="background-color:#00CF00">4 Выражения и операции</span>

<span style="color:violet">__Выражения__</span> - последовательности операций и операндов, вычисляющие значение или обладающие побочными эффектами. Операндами могут быть идентификаторы, константы, строковые литералы и другие выражения.

## <span style="background-color:#00AB00">4.1 Простое присваивание</span>

<span style="color:violet">__Простое присваивание__</span> заменяет значение объекта, указанного в левой части, правым операндом. Значение правого операнда приводится к типу выражения присваивания.
Состоит из трёх элементов: <span style="color:orange">_левого операнда_</span> (l-value), <span style="color:orange">_операции присваивания (=)_</span> и <span style="color:orange">_правого операнда_</span> (r-value).

<span style="color:violet">_Объявления с инициализацией_</span> - инициализатор использует синтаксис присваивания, но <span style="color:orange">_не является выражением присваивания_</span> - это часть объявления (см. operations/jokes_with_assignment.c). Третья строка - собственно простое присваивание. Чтобы код скомпилировался, все идентификаторы должны быть объявлены или определены.

В выражении i = j значение читается из j и записывается в i. Поскольку оба имеют тип int, преобразование не требуется. Выражение присваивания само имеет значение - результат присваивания - и тип левого операнда.

### <span style="background-color:#008D00">l-значения и r-значения</span>

<span style="color:violet">__l-значение (lvalue)__</span> - левый операнд присваивания, <span style="color:orange">_обязательно обозначающий объект в памяти_</span>. Исторически «l» от left, но точнее - <span style="color:orange">_значение-локатор (locator value)_</span>.
Примеры l-значений: идентификаторы переменных, выражения вида *(p+4), если они ссылаются на существующий объект.

<span style="color:violet">__r-значение (rvalue)__</span> - правый операнд, <span style="color:orange">_обычное значение, не обязано идентифицировать объект_</span>. Может быть результатом вычисления, константой, значением переменной.

> <span style="background-color:red; color:black">АЛАРМ!</span> r-значение не может стоять слева от =.

### <span style="background-color:#008D00">Приведение типа при присваивании</span>

Если правый операнд имеет тип, отличный от типа левого операнда, его значение <span style="color:orange">_неявно приводится_</span> к типу левого операнда.

> <span style="background-color:red; color:black">АЛАРМ!</span> Приведение к более узкому типу может вызвать усечение. (см. operations/jokes_with_assignment.c)

## <span style="background-color:#00AB00">4.2 Оценивание</span>

<span style="color:violet">__Оценивание (evaluation)__</span> - процесс упрощения выражения до одного значения, который <span style="color:orange">_может сопровождаться побочными эффектами_</span>. Включает два взаимосвязанных аспекта: <span style="color:violet">_расчёт значения_</span> и <span style="color:violet">_побочные эффекты_</span>.

### <span style="background-color:#008D00">Расчёт значения (value computation)</span>

<span style="color:violet">__Расчёт значения__</span> - это определение итогового результата выражения. В ходе расчёта может потребоваться идентифицировать объект или прочитать ранее записанное в него значение.

```c
a[i] + f() + 9
```

Здесь выполняются следующие вычисления:

1. Идентифицируется объект a[i] (l-значение) и считывается его значение.
2. Вызывается функция f() - сама f как идентификатор функции не требует идентификации объекта, но её вызов даёт значение.
3. Константа 9 представляет собой r-значение.
4. Выполняется первое сложение (результат a[i] + f()), затем второе сложение с 9.

Расчёт значений операндов всегда происходит до выполнения операции. Если a[i] - массив типа int, а f() возвращает int, то тип всего выражения - int.

### <span style="background-color:#008D00">Побочные эффекты (side effects)</span>

<span style="color:violet">__Побочные эффекты__</span> - это изменения состояния среды выполнения, возникающие при оценивании выражения. К ним относятся:

1. запись значения в объект;
2. обращение к объекту с квалификатором volatile;
3. ввод/вывод;
4. присваивание;
5. вызов функции, которая выполняет любое из перечисленных действий.
...
Здесь <span style="color:orange">_присваивание значения переменной j_</span> - побочный эффект, меняющий состояние среды. Кроме того, вызов f() сам по себе может обладать побочными эффектами (например, изменять глобальную переменную или осуществлять вывод).

> <span style="background-color:red; color:black">АЛАРМ!</span> Порядок вычисления операндов большинства операторов в C не специфицирован (за исключением некоторых). Если выражение содержит несколько побочных эффектов, влияющих на одни и те же объекты, результат может быть неопределённым. Старайтесь избегать выражений вроде <span style="color:orange">_i++ + ++i_</span>.

## <span style="background-color:#00AB00">4.3 Вызов функции</span>

<span style="color:violet">__Обозначение функции__</span> - это выражение функционального типа, используемое для вызова функции. В примере ниже обозначением функции является `max`:
...
Во время выполнения обозначение функции автоматически преобразуется в <span style="color:violet">__указатель на функцию__</span> соответствующего типа.

Значение каждого аргумента должно иметь тип, который можно присвоить объекту с (неквалифицированным) типом соответствующего параметра. Количество и типы аргументов обязаны совпадать с количеством и типами параметров функции. Язык C также поддерживает <span style="color:violet">__вариативные функции__</span> - функции, способные принимать переменное число аргументов (классический пример - `printf`).

### <span style="background-color:#008D00">Передача функции как аргумента</span>

Одну функцию можно передать другой - для этого используется указатель на функцию. При передаче имя функции автоматически преобразуется в указатель.
...
// ---snip---
g(f); // f неявно преобразуется в указатель на функцию
// ---snip---
```

<span style="color:orange">_Функция `g` принимает указатель на функцию, которая не имеет параметров и возвращает `int`._</span>  
Эквивалентным объявлением `g` могло бы быть `void g(int func(void))` - форма с прототипом функции как параметра, но оно всё равно означает указатель на функцию.

> <span style="background-color:red; color:black">АЛАРМ!</span> При вызове функции через указатель убедитесь, что сигнатура (типы параметров и возврата) полностью совпадает. Несоответствие ведёт к неопределённому поведению.

## <span style="background-color:#00AB00">4.4 Операции инкремента и декремента</span>

<span style="color:violet">__Операция инкремента (`++`)__</span> увеличивает на 1, а <span style="color:violet">__операция декремента (`--`)__</span> уменьшает на 1 свой операнд - <span style="color:orange">_изменяемое l-значение_</span>. Обе операции <span style="color:orange">_унарные_</span> (принимают один операнд).

Операции бывают двух форм:
* <span style="color:violet">__Префиксная__</span>: `++i`, `--i` - значение изменяется **до** того, как будет возвращено выражением.
* <span style="color:violet">__Постфиксная__</span>: `i++`, `i--` - значение изменяется **после** того, как будет возвращено выражением.

### <span style="background-color:#008D00">Поведение префиксной и постфиксной форм</span>

```c
int i = 5;
int e; // результат выражения

e = i++; // постфиксный инкремент: i становится 6, e получает 5
e = i--; // постфиксный декремент: i становится 5, e получает 6
e = ++i; // префиксный инкремент:  i становится 6, e получает 6
e = --i; // префиксный декремент:  i становится 5, e получает 5
```

* `i++` возвращает **старое** значение `5` и затем увеличивает `i` до `6`.
* `++i` сначала увеличивает `i` до `6`, а затем возвращает **новое** значение.
  Выражение `++i` эквивалентно `i = i + 1`, но <span style="color:orange">_`i` вычисляется только один раз_</span>.

> <span style="background-color:red; color:black">АЛАРМ!</span> Операндом **обязательно** должно быть изменяемое l-значение (переменная, элемент массива, разыменованный указатель). Попытка применить `++` или `--` к r-значению (например, `5++`) - ошибка компиляции.

> <span style="background-color:red; color:black">АЛАРМ!</span> Избегайте использования нескольких операций инкремента/декремента над одной переменной в одном выражении (например, `i++ + ++i`). Порядок вычисления побочных эффектов в C не определён, что ведёт к неопределённому поведению, а не к надёжному коду.

## <span style="background-color:#00AB00">4.5 Приоритет и ассоциативность операций</span>

<span style="color:violet">__Приоритет операций__</span> определяет, в каком порядке выполняются операции в выражении при отсутствии явных скобок. Операции с более высоким приоритетом выполняются раньше.  
<span style="color:violet">__Ассоциативность__</span> определяет порядок группирования операций с одинаковым приоритетом:  
* <span style="color:orange">_левоассоциативные_</span> группируются слева направо (например, `a + b + c` → `(a + b) + c`),  
* <span style="color:orange">_правоассоциативные_</span> - справа налево (например, `a = b = c` → `a = (b = c)`).

### <span style="background-color:#008D00">4.5.1 Таблица приоритетов и ассоциативности</span>

| Приоритет | Операция | Описание | Ассоциативность |
|-----------|----------|----------|-----------------|
| 0 | `()` | Принудительное группирование | Слева направо |
| 1 | `++` `--` | Постфиксные инкремент/декремент | Слева направо |
| | `()` | Вызов функции | |
| | `[]` | Обращение к элементу массива | |
| | `.` | Доступ к члену структуры/объединения | |
| | `->` | Доступ к члену через указатель | |
| | `(тип){список}` | Составной литерал (C99) | |
| 2 | `++` `--` | Префиксные инкремент/декремент | Справа налево |
| | `+` `-` | Унарные плюс и минус | |
| | `!` `~` | Логическое и битовое НЕ | |
| | `(тип)` | Приведение типа | |
| | `*` | Разыменование | |
| | `&` | Взятие адреса | |
| | `sizeof` | Размер | |
...
| 14 | `=` `+=` `-=` `*=` `/=` `%=` `<<=` `>>=` `&=` `^=` `\|=` | Присваивание и составные присваивания | Справа налево |
| 15 | `,` | Операция «запятая» (последовательность) | Слева направо |

Примеры операций в файле operations/jokes_with_associations.c

> <span style="background-color:red; color:black">АЛАРМ!</span> Приоритет операций не всегда интуитивен. Если есть сомнения - <span style="color:orange">_расставляйте скобки явно_</span>. Это не только гарантирует нужный порядок вычислений, но и делает код понятнее для читателя. Не стесняйтесь использовать `()` даже там, где они формально не обязательны.

## <span style="background-color:#00AB00">4.6 Порядок вычисления</span>

<span style="color:violet">__Порядок вычисления__</span> операндов в любой операции языка C (включая вложенные выражения) <span style="color:orange">_обычно не специфицирован_</span>. Компилятор может выбирать любой порядок, и он может меняться от вызова к вызову. Единственные ограничения накладывают приоритет и ассоциативность операций. Такая свобода позволяет компилятору генерировать более эффективный код.

Пример с объяснением (частично затронуты разделы про [непоследовательные вычисления](#461-непоследовательные-и-неопределённо-последовательные-вычисления) и [точки следования](#462-точки-следования))

### <span style="background-color:#008D00">4.6.1 Непоследовательные и неопределённо последовательные вычисления</span>

* <span style="color:violet">__Непоследовательные вычисления__</span> могут <span style="color:orange">_перемежаться_</span> (инструкции одного вычисления могут выполняться вперемешку с инструкциями другого), но при этом должны соблюдаться согласованные порядки чтения/записи, заданные программой.
* <span style="color:violet">__Неопределённо последовательные вычисления__</span> <span style="color:orange">_не перемежаются_</span>, но могут выполняться в любом порядке.

Пример с неопределённо последовательными вычислениями:

```c
printf("%d\n", ++i + ++j * --k);
```

Здесь чтение и побочные эффекты (инкременты, декременты) могут происходить в любом порядке, который не нарушает зависимости: например, чтение `i` должно предшествовать его инкременту, умножение не может начаться до получения побочных эффектов операндов, сложение — после умножения. Однако порядок между инкрементом `i` и декрементом `k` не определён, что позволяет компилятору свободно переставлять их и кешировать значения в регистрах.

### <span style="background-color:#008D00">4.6.2 Точки следования</span>

<span style="color:violet">__Точка следования__</span> - момент, к которому завершаются все побочные эффекты предыдущих вычислений. Точки следования:

* между двумя <span style="color:violet">__полными выражениями__</span> (выражениями, не являющимися частью другого выражения или объявления);
* на входе в вызываемую функцию и при выходе из неё.

> <span style="background-color:red; color:black">АЛАРМ!</span> Если два побочных эффекта, действующие на один и тот же <span style="color:violet">__скалярный объект__</span> (арифметический тип или указатель), не упорядочены относительно друг друга, поведение не определено.

Классический пример неопределённого поведения:
...
Два неупорядоченных побочных эффекта (`i++`) модифицируют один и тот же скаляр `i`, результат непредсказуем.

Исправленная версия вводит точки следования:

```c
int i = 5;
int j = i++;
int k = i++;
printf("Result = %d\n", j * k); // точки следования гарантируют порядок
```
...
В этих вариантах каждый побочный эффект завершается до начала следующего полного выражения, поведение строго определено.

## <span style="background-color:#00AB00">4.7 Операция sizeof</span>

<span style="color:violet">__sizeof__</span> - унарная операция, возвращающая размер операнда в байтах. Результат имеет тип <span style="color:violet">__size_t__</span> (беззнаковое целое) и является <span style="color:orange">_константным выражением_</span>, если операнд - не массив переменной длины. Тип `size_t` определён в `<stddef.h>` и других стандартных заголовках; для его использования один из этих файлов должен быть подключён.

Операндом может быть:
- выражение объектного типа (не вычисляется!),
- имя типа, заключённое в скобки.
...
Скобки вокруг операнда‑выражения безопасны и не меняют семантики. <span style="color:orange">_Операнд sizeof не вычисляется_</span> (за исключением случаев VLA (массива переменной длины), где вычисляется размер массива).

Чтобы узнать размер в битах, умножают результат на <span style="color:violet">__CHAR_BIT__</span> (см. заметку про [макросы](#библиотечные-методы-и-макросы)): `CHAR_BIT * sizeof(int)` даст количество битов в объекте `int`.

> <span style="background-color:red; color:black">АЛАРМ!</span> Все объектные типы, кроме символьных (`char`, `signed char`, `unsigned char`), могут содержать <span style="color:violet">__биты заполнения__</span> - неиспользуемые биты, не участвующие в представлении значения.

Разные платформы по-разному упаковывают байты в многобайтные слова (порядок байтов). Для передачи объектов по сети необходимо:
1. Договориться об общем внешнем формате представления.
2. Использовать функции маршалинга (упаковки) и демаршалинга (распаковки), преобразующие объекты между платформозависимым представлением и этим форматом.

## <span style="background-color:#00AB00">4.8 Арифметические операции</span>

### <span style="background-color:#008D00">4.8.1 Унарные операции + и –</span>

<span style="color:violet">__Унарный минус (`–`)__</span> возвращает отрицательное значение операнда (как если бы его умножили на –1).  
<span style="color:violet">__Унарный плюс (`+`)__</span> просто возвращает значение операнда.

Обе операции применяются к операндам числовых типов. Если операнд имеет малый целочисленный тип (ранг ниже `int`), его разрядность повышается (см. [главу 3](#33-арифметическое-преобразование)), и результат получает расширенный тип.

Исторически в C нет отрицательных целочисленных констант: запись `-25` - это <span style="color:orange">_r-значение_</span> типа `int` (величина 25), перед которым стоит унарный `-`.

### <span style="background-color:#008D00">Логическое отрицание (`!`)</span>

<span style="color:violet">__Логическое НЕ (`!`)__</span> - унарная операция, возвращающая:
- `0`, если операнд не равен 0,
- `1`, если операнд равен 0.

Операнд должен иметь скалярный тип, результат всегда типа <span style="color:orange">_int_</span>.  
Выражение `!E` эквивалентно `(0 == E)`. Часто применяется для проверки на нулевой указатель: <span style="color:orange">_!p_</span> равносильно <span style="color:orange">_(NULL == p)_</span>.

### <span style="background-color:#008D00">4.8.2 Мультипликативные операции (`*`, `/`, `%`)</span>

К двоичным мультипликативным операциям относятся:
- <span style="color:violet">__умножение (`*`)__</span>,
- <span style="color:violet">__деление (`/`)__</span>,
- <span style="color:violet">__взятие остатка (`%`)__</span>.

Операнды подчиняются обычным арифметическим преобразованиям. Умножение и деление работают и с целыми, и с плавающей точкой; взятие остатка - только с целочисленными операндами.

В языке C реализовано <span style="color:orange">_деление с усечением (truncation toward zero)_</span>. Частное - алгебраическое частное без дробной части. Остаток всегда имеет тот же знак, что и делимое. Также есть еще два вида деления, не представленные в языке C:

- <span style="color:violet">__евклидовое деление__</span> - остаток всегда получается неотрицательным;
- <span style="color:violet">__деление с возвращением наименьшего целого__</span> - частное округляется в сторону минус бесконечности.
...
Если частное `a / b` представимо, то `(a / b) * b + a % b` эквивалентно `a`.

> <span style="background-color:red; color:black">АЛАРМ!</span> Деление на ноль или переполнение при делении (например, `INT_MIN / -1`) вызывает неопределённое поведение. То же относится и к операции `%` в аналогичных ситуациях. Смотри файл operations/jokes_with_ariphmetic_operations.c

Из-за того что остаток сохраняет знак делимого, следующая проверка на нечётность некорректна для отрицательных чисел:

```c
bool is_odd(int n) {
    return n % 2 == 1;   // ошибка: для отрицательного нечётного вернёт false
}
```

Корректная версия проверяет, не равен ли остаток нулю:
...
Стандартная библиотека предоставляет функцию <span style="color:violet">__fmod__</span> для взятия остатка с плавающей запятой.

### <span style="background-color:#008D00">Аддитивные операции (`+` и `–`)</span>

К бинарным аддитивным операциям относят сложение (`+`) и вычитание (`–`). Они применяются к двум операндам числовых типов, а также могут использоваться для арифметики указателей. К операндам числовых типов применяются обычные арифметические преобразования.

## <span style="background-color:#00AB00">4.9 Битовые операции</span>

Битовые операции управляют отдельными битами целочисленных значений. Их применяют к <span style="color:violet">__битовым картам__</span> - объектам, где каждый бит указывает на состояние «включено/выключено», «установлен/сброшен» и т.п.  
Для битовых карт предпочтительны <span style="color:orange">_беззнаковые целочисленные типы_</span>: знаковый разряд используется как полноценный бит данных, а поведение операций меньше подвержено неопределённости.

> Смотри примеры в operations/jokes_with_bit_operations.c

### <span style="background-color:#008D00">4.9.1 Операция дополнения (`~`)</span>

<span style="color:violet">__Операция дополнения (битовая инверсия, `~`)__</span> - унарная, принимает целочисленный операнд и возвращает его побитовое дополнение (каждый бит инвертирован). Операнд подвергается повышению разрядности, результат имеет расширенный тип.

> <span style="background-color:red; color:black">АЛАРМ!</span> Во избежание сюрпризов битовые операции всегда выполняйте с одним беззнаковым целочисленным типом достаточной ширины.

### <span style="background-color:#008D00">4.9.2 Операции сдвига (`<<` и `>>`)</span>

* <span style="color:violet">__Сдвиг влево (`E1 << E2`)__</span> - сдвигает биты `E1` влево на `E2` позиций, освободившиеся правые биты заполняются нулями.
* <span style="color:violet">__Сдвиг вправо (`E1 >> E2`)__</span> - сдвигает биты `E1` вправо на `E2` позиций.

Для `a` справедливо:

1. для безнакового a - <span style="color:orange">_a << b = a*(2^b)_</span> (с циклическим переносом при выходе за диапазон) или <span style="color:orange">_a >> b = a/(2^b)_</span> для неотрицательного a (в т.ч. знакового типа)
2. для знакового a - <span style="color:orange">_a << b = a*(2^b)_</span>, если результат представим; иначе поведение не определено. Правый сдвиг отрицательного знакового `a` <span style="color:orange">_определяется реализацией_</span> (может быть арифметическим - с распространением знакового бита, или логическим - с заполнением нулями).

Операнды подвергаются повышению разрядности. Результат имеет тип расширенного левого операнда. Обычные арифметические преобразования не применяются.

> <span style="background-color:red; color:black">АЛАРМ!</span> Количество сдвигаемых битов (`b`) <span style="color:orange">_не должно быть отрицательным_</span>, а также не должно быть больше или равно ширине расширенного левого операнда - иначе неопределённое поведение.

Примеры (смотри еще в operations/jokes_with_bit_operations.c)

```c
extern int si1, si2, sresult;
extern unsigned int ui1, ui2, uresult;

// Знаковый сдвиг вправо: проверяем и на отрицательность, и на ширину
...
// Беззнаковый сдвиг вправо: достаточно проверить верхнюю границу
...
Не используй сдвиги для умножения/деления на степень двойки - это преждевременная оптимизация. Такую оптимизацию лучше доверить компилятору, понял?

### <span style="background-color:#008D00">4.9.3 Битовое И (`&`)</span>

<span style="color:violet">__Битовое И (`&`)__</span> - бинарная операция над целочисленными операндами. Каждый бит результата равен 1, только если оба соответствующих бита операндов равны 1. К операндам применяются обычные арифметические преобразования.
...
### <span style="background-color:#008D00">4.9.4 Битовое исключающее ИЛИ (`^`)</span>

<span style="color:violet">__Битовое исключающее ИЛИ (XOR, `^`)__</span> - бинарная операция над целочисленными операндами. Каждый бит результата равен 1, только если соответствующие биты операндов различны ("либо тот, либо другой, но не оба сразу"). Эквивалентна сложению по модулю 2. Операнды проходят обычные арифметические преобразования.
...
> <span style="background-color:red; color:black">АЛАРМ!</span> `^` - не возведение в степень. Для возведения в степень используйте функции <span style="color:violet">__pow__</span> из `<math.h>`. Они работают с числами с плавающей запятой, что может привести к погрешностям.

### <span style="background-color:#008D00">4.9.5 Битовое включающее ИЛИ (`|`)</span>

<span style="color:violet">__Битовое включающее ИЛИ (`|`)__</span> - бинарная операция над целочисленными операндами. Каждый бит результата равен 1, если хотя бы один из соответствующих битов операндов равен 1. Операнды проходят обычные арифметические преобразования.
...
## <span style="background-color:#00AB00">4.10 Логические операции</span>

<span style="color:violet">__Логическое И (`&&`)__</span> и <span style="color:violet">__логическое ИЛИ (`||`)__</span> - бинарные операции, работающие с операндами скалярных типов и возвращающие значение типа `int`:
* a && b - 1, если <span style="color:orange">_оба_</span> операнда не равны 0, иначе 0;
* a || b - 1, если <span style="color:orange">_хотя бы один_</span> операнд не равен 0, иначе `0`.

Обе операции определены через сравнение с нулём, поэтому операнды могут быть <span style="color:orange">_не только 0/1_</span>, но и, например, указателями или числами с плавающей запятой.

> <span style="background-color:red; color:black">АЛАРМ!</span> Логические операции не предназначены для работы с битовыми картами. Для поразрядных операций используйте `&`, `|`, `~`.

### <span style="background-color:#008D00">4.10.1 Порядок вычисления и короткая схема</span>

Логические И и ИЛИ гарантируют <span style="color:orange">_лево-правый порядок_</span> вычисления операндов; между первым и вторым операндом находится <span style="color:orange">_точка следования_</span>.  Работает <span style="color:violet">__короткая схема (short-circuit evaluation)__</span>: если результат ясен после вычисления левого операнда, правый <span style="color:orange">_не вычисляется вовсе_</span>.

* 0 && expr → 0, expr не вычисляется.
* 1 || expr → 1, expr не вычисляется.

Это даёт два практических преимущества:

1. <span style="color:orange">_Безопасность_</span>: можно проверить указатель перед разыменованием.  
   ```c
   bool isN(int* ptr, int n) {
       return ptr && *ptr == n; // если ptr == NULL, *ptr не вычисляется
   }
   ```
2. <span style="color:orange">_Производительность_</span>: можно избежать дорогих операций, когда они не нужны.  
   ```c
   if (is_file_ready() || prepare_file()) { /* ... */ }
   // prepare_file() вызывается, только если is_file_ready() вернула false
   ```

> <span style="background-color:red; color:black">АЛАРМ!</span> Будьте осторожны, если второй операнд содержит побочные эффекты - их выполнение не гарантируется.  
> Пример рискованного кода:  
> ```c
> if ((i >= 0) && ((i++) <= max)) { /* ... */ }
> // i++ выполнится, только если i >= 0
> ```
> Такой код трудно читать и сопровождать; по возможности выносите побочные эффекты за пределы логических выражений.

## <span style="background-color:#00AB00">4.11 Операции приведения типов</span>

<span style="color:violet">__Приведение типов (cast)__</span> - явное преобразование значения из одного типа в другой. Синтаксис: <span style="color:orange">_(тип) выражение_</span>. Результат - значение выражения, приведённое к указанному типу (неквалифицированной версии, если тип был квалифицирован).

```c
double x = 1.2;
int sum = (int)x + 1;   // double → int, дробная часть отбрасывается
```

Допустимые преобразования:
- И тип, и операнд должны быть <span style="color:orange">_скалярными_</span> (кроме void).
- Указатель нельзя приводить к типу с плавающей запятой, и наоборот.

### <span style="background-color:#008D00">4.11.1 Два сценария приведения</span>

1. <span style="color:violet">__Переинтерпретация битов__</span> - битовый набор остаётся прежним, но интерпретируется как значение другого типа.  
   ```c
   intptr_t i = (intptr_t)some_pointer;  // биты указателя стали целым числом
   ```
2. <span style="color:violet">__Преобразование значения__</span> — биты могут измениться так, чтобы представить исходную величину в целевом типе.  
   ```c
   int i = (int)some_float;   // математическое преобразование, может потерять точность
   ```

> <span style="background-color:red; color:black">АЛАРМ!</span> Приведение типов - мощный, но опасный инструмент. Оно способно скрывать диагностику компилятора, не устраняя саму проблему.

Классический пример - подавление предупреждения о возможной потере данных при присваивании результата fgetc (тип int) переменной типа char:

```c
char c;
while ((c = fgetc(in)) != EOF) {   // warning: int → char
    // ...
}
```

Добавление явного приведения убирает предупреждение, но не решает проблему: значение <span style="color:orange">_EOF_</span> (обычно -1) может быть неотличимо от допустимого символа (например, 0xFF), если <span style="color:orange">_char_</span> беззнаковый или если его диапазон не вмещает EOF.

```c
while ((c = (char)fgetc(in)) != EOF) {  // тишина, но ошибка осталась
    // ...
}
```

<span style="color:orange">_Никогда не используйте_</span> приведение типов только для того, <span style="color:orange">_чтобы заглушить предупреждения_</span> компилятора. Сначала убедитесь, что преобразование действительно безопасно.

## <span style="background-color:#00AB00">4.12 Условная операция</span>

<span style="color:violet">__Условная операция (`?:`)__</span> - единственная операция в C, принимающая три операнда (тернарная). Синтаксис:
...
Порядок вычислений:
1. Вычисляется первый операнд - <span style="color:violet">__условие__</span> (скалярного типа).
2. Если оно не равно нулю (истина), вычисляется второй операнд, а третий игнорируется.
3. Если равно нулю (ложь), вычисляется третий операнд, а второй игнорируется.
4. Результат операции - значение вычисленного (второго или третьего) операнда, приведённое к общему типу, который определяется по типам второго и третьего операндов согласно правилам обычных арифметических преобразований.

Между вычислением условия и вычислением второго/третьего операнда находится <span style="color:orange">_точка следования_</span>: все побочные эффекты условия гарантированно завершены до перехода к оставшейся части.

Условная операция похожа на if-else, но возвращает значение и может использоваться в выражениях, в том числе для инициализации константных объектов:

```c
const int x = (a < b) ? b : a;   // x получает большее из a и b
```

> <span style="background-color:red; color:black">АЛАРМ!</span> Типы второго и третьего операндов должны быть совместимы (подробные правила в разделе 6.5.15 стандарта C). Если они существенно различаются, результат приведения может оказаться неожиданным. Избегайте смешивания указателей и целых чисел, а также резко различающихся арифметических типов без явного приведения.

## <span style="background-color:#00AB00">4.13 Операция _Alignof</span>

<span style="color:violet">__\_Alignof__</span> - унарная операция (начиная с C11), возвращающая <span style="color:orange">_целочисленную константу_</span> - требование к выравниванию для указанного полного объектного типа. Операнд (тип или выражение) не вычисляется. Если операнд - массив, результат даёт выравнивание его элемента. Доступен также как удобный макрос <span style="color:violet">__alignof__</span> из заголовка stdalign.h.

Операция полезна для статических и динамических утверждений (см. главу 11), позволяющих диагностировать неверные предположения о выравнивании (см. operations/jokes_with_alignof_operation.c)

<span style="color:orange">_`max_align_t`_</span> - тип с максимально строгими требованиями выравнивания в данной реализации. 

> <span style="background-color:red; color:black">АЛАРМ!</span> Если динамические утверждения отключены <span style="color:orange">_макросом NDEBUG, вызов assert <span style="color:orange">_не компилируется_</span>, и несоответствие выравнивания может остаться незамеченным. <span style="color:orange">_Статические утверждения_</span> (static_assert) срабатывают на этапе компиляции всегда, независимо от NDEBUG.

## <span style="background-color:#00AB00">4.14 Операции сравнения</span>

К операциям сравнения относятся:

* <span style="color:violet">__==__</span> (равно),
* <span style="color:violet">__!=__</span> (не равно),
* <span style="color:violet">__\<__</span> (меньше),
* <span style="color:violet">__\>__</span> (больше),
* <span style="color:violet">__<=__</span> (меньше или равно),
* <span style="color:violet">__>=__</span> (больше или равно).

Каждая возвращает <span style="color:orange">_1_</span>, если сравнение истинно, и <span style="color:orange">_0_</span>, если ложно. Результат имеет тип <span style="color:orange">_int_</span> .

### <span style="background-color:#008D00">4.14.1 Особенности приоритета и группировки</span>

Операции <, >, <=, >= имеют более высокий приоритет, чем == и !=. Это частая ловушка:

```c
a < b == c < d   // означает (a < b) == (c < d)
```

<span style="color:orange">_Всегда используйте скобки_</span>, чтобы явно задать намерение.

Другая классическая ошибка - попытка записать двойное неравенство как в математике - так не работает:

```c
a < b < c        // означает (a < b) < c, то есть сравнивает 0 или 1 с c
```
...
> <span style="background-color:red; color:black">АЛАРМ!</span> Некоторые компиляторы (GCC, Clang) предоставляют флаг <span style="color:orange">_-Wparentheses_</span>, который предупреждает о подозрительной расстановке скобок. 

### <span style="background-color:#008D00">4.14.2 Сравнение указателей</span>

Операции сравнения применимы и к указателям. При этом:

* == и != допустимы всегда: два указателя равны, только <span style="color:orange">_если указывают на один и тот же объект_</span> (или оба нулевые).
* <, >, <=, >= имеют смысл только если указатели ссылаются на элементы одного массива или на один и тот же объект. Сравнение указателей на разные объекты - <span style="color:orange">_неопределённое поведение_</span>.

```c
int i, j;
bool b1 = &i < &j;   // НЕОПРЕДЕЛЁННОЕ ПОВЕДЕНИЕ
bool b2 = &i == &j;  // корректно, но всегда false (тавтология)
```

> <span style="background-color:red; color:black">АЛАРМ!</span> Не используйте операции порядка для указателей на разные объекты. Если нужно сравнить адреса как числа, сначала приведите их к `uintptr_t` (из `<stdint.h>`). Для надёжного сравнения на равенство используйте `==` или `!=`.

## <span style="background-color:#00AB00">4.15 Операции составного присваивания</span>

<span style="color:violet">__Составное присваивание__</span> - это краткая форма, совмещающая арифметическую (или битовую) операцию с присваиванием. Вместо того чтобы писать <span style="color:orange">_a = a operation b_</span>, используют запись <span style="color:orange">_a operation= b_</span>. При этом a вычисляется <span style="color:orange">_только один раз_</span>.

Перечень операций составного присваивания:

| Операция | Описание |
|----------|----------|
| `+=` `-=` | Присваивание с суммой/разностью |
| `*=` `/=` `%=` | Присваивание с произведением/частным/остатком |
| `<<=` `>>=` | Присваивание со сдвигом влево/вправо |
| `&=` `^=` `\|=` | Присваивание с битовыми И, исключающее ИЛИ, ИЛИ |

Логических операций среди составных присваиваний нет (`&&=` и `||=` не существуют).

## <span style="background-color:#00AB00">4.16 Операция «запятая»</span>

<span style="color:violet">__Операция "запятая" (,)__</span> - бинарная операция, позволяющая последовательно вычислить два выражения там, где синтаксис допускает только одно, либо просто разделить элементы, например, массива.

Порядок работы:
1. Вычисляется левый операнд, его результат отбрасывается (вычисляется как void-выражение).
2. Между вычислением левого и правого операндов находится <span style="color:orange">_точка следования_</span>.
3. Вычисляется правый операнд; его значение и тип становятся результатом всей операции.

<span style="color:orange">_Запятая-операция не должна путаться с запятой-разделителем_</span> (списки аргументов, объявления). Там, где запятая могла бы трактоваться как разделитель, выражение с операцией "запятая" необходимо заключать в скобки.
...
Здесь:
- Первая запятая - разделитель аргументов (a и следующего выражения).
- Вторая запятая внутри скобок - операция "запятая": сначала выполняется t = 3 (побочный эффект), затем t + 2, результат (5) становится вторым аргументом.
- Третья запятая - разделитель между вторым и третьим аргументами (c).

> <span style="background-color:red; color:black">АЛАРМ!</span> Операция "запятая" легко делает код трудночитаемым. Хороший тон - применять запятую только в заголовках циклов (for) и макросах, где без неё не обойтись.

## <span style="background-color:#00AB00">4.17 Арифметические операции с указателями</span>

Аддитивные операции (+ и -) можно применять к указателям на объекты. Поддерживаются три формы:
- сложение указателя и целого числа,
- вычитание из указателя целого числа,
- вычитание одного указателя из другого.

### <span style="background-color:#008D00">Сложение/вычитание указателя и целого</span>

Сумма или разность выражения целочисленного типа и указателя возвращает значение <span style="color:orange">_того же типа, что и указатель_</span>. Если указатель ссылается на элемент массива, результат указывает на элемент, смещённый на соответствующее количество позиций. При этом разница между индексами исходного и итогового элементов равна целочисленному выражению.

```c
int arr[100];
int *arrp1 = &arr[40];           // arrp1 указывает на arr[40]
int *arrp2 = arrp1 + 20;        // arrp2 указывает на arr[60]
printf("%td\n", arrp2 - arrp1); // выводит 20
```

> <span style="background-color:red; color:black">АЛАРМ!</span> Если итоговый указатель выходит за пределы массива, поведение не определено.

### <span style="background-color:#008D00">4.17.1 «Слишком дальний» указатель</span>

В C разрешено формировать указатель на <span style="color:violet">__несуществующий элемент, следующий за последним элементом массива__</span> - так называемый «слишком дальний» указатель. Разыменовывать его нельзя, но использовать в сравнениях и арифметике допустимо.

Если операнд-указатель и результат операции оба указывают на элементы одного массива или являются «слишком дальними», вычисление не переполняется; иначе поведение не определено. Чтобы соблюсти это требование, реализации достаточно предоставить один дополнительный байт сразу после объекта.

Любой объект можно рассматривать как массив из одного элемента, поэтому «слишком дальний» указатель допустим и для скалярных значений.
...
Условие <span style="color:orange">_pi < &m[2]_</span> истинно, пока pi не станет «слишком дальним» указателем - тогда цикл завершается.

### <span style="background-color:#008D00">Разность двух указателей</span>

При вычитании одного указателя из другого оба должны ссылаться на элементы одного массива (или на «слишком дальний» элемент). Операция возвращает разность индексов, результат имеет знаковый целочисленный тип <span style="color:violet">__ptrdiff_t__</span>.

> <span style="background-color:red; color:black">АЛАРМ!</span> Диапазона ptrdiff_t может не хватить для представления разности указателей на элементы очень больших массивов char.

Арифметические операции с указателями автоматически учитывают размер элементов; они работают не с отдельными байтами, а с шагом, равным размеру типа, на который ссылается указатель.

# <span style="background-color:#00CF00">5 Управляющая логика</span>

## <span style="background-color:#00AB00">5.1 Операторы-выражения</span>

<span style="color:violet">__Оператор-выражение__</span> - произвольное выражение, заканчивающееся точкой с запятой. Это элементарная операция, один из самых распространённых видов выражений.

Для примера смотри файл control_logic/jokes_with_expression_operators.c

> <span style="color:violet">__нулевой оператор__</span> (пустой оператор). Используется, когда синтаксис требует наличия оператора, но никаких действий выполнять не нужно: заполнители при итерировании, метки в конце блоков или функций.

После вычисления каждого полного выражения его значение (если есть) отбрасывается, так что любой полезный результат достигается исключительно через <span style="color:orange">_побочные эффекты_</span> (см. главу 4). После завершения всех эффектов выполнение переходит к оператору, следующему за точкой с запятой.

## <span style="background-color:#00AB00">5.2 Составные операторы</span>

<span style="color:violet">__Составной оператор__</span> (или <span style="color:violet">__блок__</span>) - список из любого количества операторов (в том числе нулевого), заключённых в фигурные скобки. В блоке могут находиться любые операторы, описанные в данной главе, а также объявления (в современных версиях C объявления внутри блока могут располагаться в любом месте, а не только в начале). Операторы внутри блока выполняются последовательно, если только их порядок не изменяется управляющим оператором. После выполнения последнего оператора управление переходит к коду, расположенному сразу за закрывающей фигурной скобкой.

Составные операторы могут быть <span style="color:orange">_вложенными_</span> (один блок полностью находится внутри другого). Допускаются и пустые блоки - просто пара фигурных скобок `{}`.

### <span style="background-color:#008D00">5.2.1 Стиль кода</span>

Расположение фигурных скобок <span style="color:orange">_варьируется от стиля к стилю_</span>. При работе в существующем проекте следует придерживаться его стиля. В противном случае можно выбрать один из распространённых вариантов:

- открывающая и закрывающая скобки выровнены по горизонтали (каждая на отдельной строке), чтобы легче находить пары;
- стиль Кернигана и Ритчи: открывающая скобка в конце предыдущей строки, закрывающая — на отдельной строке.

## <span style="background-color:#00AB00">5.3 Операторы выбора</span>

Операторы выбора позволяют выполнять вложенные операторы в зависимости от значения управляющего выражения. К операторам выбора относятся <span style="color:violet">__if__</span> и <span style="color:violet">__switch__</span>.

### <span style="background-color:#008D00">5.3.1 Оператор if</span>

Оператор <span style="color:orange">_if_</span> выполняет другие операторы в зависимости от значения управляющего выражения скалярного типа.

<span style="color:violet">__Формы оператора if:__</span>

*   <span style="color:orange">_Без else_</span>:
    ```c
    if (выражение)
        подоператор
    ```
    Подоператор выполняется, если выражение не равно 0. Условно выполняется только один оператор (который может быть составным блоком).

*   <span style="color:orange">_С else:_</span>
    ```c
    if (выражение)
        подоператор1
    else
        подоператор2
    ```
    Выполняется либо подоператор1 (если выражение ≠ 0), либо подоператор2. Только один из них.

Вложенные операторы <span style="color:orange">_if...else_</span> позволяют выбрать один из нескольких вариантов:
...
Выполняется ровно один подоператор: первый, чьё условие истинно, либо последний (else), если все ложны.  

<span style="color:violet">__Ловушка: пропущенные фигурные скобки__</span>

```c
if (condition)
    conditionally_executed_function();
    second_conditionally_executed_function(); // всегда выполняется!
```

Отступ не влияет на логику; без скобок if управляет только одним следующим оператором.  
Решение - всегда заключать несколько операторов в фигурные скобки или размещать одиночный оператор на одной строке с if (if (!quotient) return false;).

> Для примеров перейди в файл control_logic/jokes_with_choices_operators.c

<span style="background-color:red; color:black">АЛАРМ!</span> GCC с флагом `-Wmisleading-indentation` предупреждает о несовпадении отступов с логикой.

### <span style="background-color:#008D00">5.3.2 Оператор switch</span>

Оператор <span style="color:violet">__switch__</span> передаёт управление одной из нескольких ветвей в зависимости от значения управляющего выражения целочисленного типа.
...
*   Управляющее выражение подвергается <span style="color:orange">_повышению разрядности_</span>.
*   Константы в case приводятся к этому расширенному типу.
*   При совпадении выполнение начинается с соответствующей метки case и продолжается <span style="color:orange">_последовательно через все следующие метки_</span>, пока не встретится break или конец switch` («проваливание»).
*   Если совпадений нет, выполняется default` (если есть). Иначе не выполняется ни одна ветвь.
*   Метки case и default относятся <span style="color:orange">_только к ближайшему_</span> объемлющему switch.

<span style="background-color:red; color:black">АЛАРМ!</span> Пропуск break - частая ошибка. GCC с флагом <span style="color:orange">_-Wimplicit-fallthrough_</span> предупреждает о возможном непреднамеренном проваливании. В C2x появится атрибут <span style="color:orange">_[[fallthrough]]_</span> для явного обозначения намеренного провала.

Рекомендации по использованию:
*   Если перечисление используется в switch, всегда <span style="color:orange">_добавляйте метк_</span> default для обнаружения необработанных значений (например, 
```c
default: abort();
```
).
*   Компиляторы (Clang, GCC) с флагом <span style="color:orange">_-Wswitch-enum_</span> предупреждают, если не все значения перечисления покрыты case.

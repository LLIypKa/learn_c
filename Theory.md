...
<span style="color:violet">__Порядок__</span> - степень порядкового числа в экспоненциальной записи (например, для -6,022 x 10^23, 23 - это порядок). Чтобы не хранить отдельно знак порядка, применяют <span style="color:violet">__смещение__</span> (<span style="color:violet">__bias__</span>). К реальному порядку прибавляют константу, и в память записывают эту сумму. Для float смещение равно 127 (следовательно, чтобы представить порядок 0, в поле экспоненты сохраняется 127; порядок 200 соответствует степени 200 – 127 = 73). Для double смещение равно 1023. Крайние значения экспоненты (все нули и все единицы) зарезервированы для специальных случаев (денормализованные числа, бесконечность, NaN).
...
-(1)^S * (1 + M) * 2^E, где S - знак (для всех типов - 1 бит), M - мантисса (для float - 23 бита, для double - 52 бита), E - порядок (для float - 8 бит, для double - 11 бит)
